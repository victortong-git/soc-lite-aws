...
import { RefreshCw, Clock, CheckCircle, XCircle, AlertCircle, PlayCircle, Pause, Play, Trash2, RotateCcw, X, PauseCircle } from 'lucide-react';
...
export const JobsPage: React.FC = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await apiClient.get(`/analysis-jobs?${params.toString()}`);
  ...
  const fetchStats = async () => {
    try {
      const response = await apiClient.get('/analysis-jobs/stats');
  ...
  const handleRetryJob = async (jobId: number) => {
    try {
      ...
      const response = await apiClient.put(`/analysis-jobs/${jobId}/retry`);
  ...
  const handleCancelJob = async (jobId: number) => {
    ...
    try {
      setActionLoading(true);
      const response = await apiClient.delete(`/analysis-jobs/${jobId}/cancel`);
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      switch (showConfirm.action) {
        case 'pause':
          response = await apiClient.put('/analysis-jobs/bulk/pause');
          break;
        case 'resume':
          response = await apiClient.put('/analysis-jobs/bulk/resume');
          break;
        case 'clear-completed':
          response = await apiClient.delete('/analysis-jobs/bulk/clear-completed');
          break;
        case 'clear-failed':
          response = await apiClient.delete('/analysis-jobs/bulk/clear-failed');
          break;
        case 'clear-all':
          response = await apiClient.delete('/analysis-jobs/bulk/clear-all');
          break;
      }
  ...
  const hasOnHoldJobs = jobs.some(j => j.status === 'on_hold');
    ...
    <Layout>
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-theme-text">Analysis Jobs</h2>
            <p className="mt-1 text-xs sm:text-sm text-theme-text-secondary">
              AI analysis job queue (Max 2 concurrent)
            </p>
          </div>
                ...
                return (
                  <div key={job.id} className="card p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div>
                        <a href={`/events/${job.event_id}`} className="text-primary-600 dark:text-primary-400 hover:underline">
                          Event #{job.event_id}
                        </a>
                      </div>
                      ...
                      {job.status === 'completed' && job.severity_rating !== undefined && (
                        <div className="flex justify-between">
                          <span className="text-theme-text-secondary">Severity:</span>
                          <span className="font-medium text-theme-text">{job.severity_rating}/10</span>
                        </div>
                      )}
                          ...
                          {job.status === 'completed' && job.severity_rating !== undefined ? (
                            <div className="text-sm">
                              <div className="font-medium text-theme-text">
                                Severity: {job.severity_rating}/10
                              </div>
                              {job.security_analysis && (
                                <div className="text-xs text-theme-text-secondary mt-1 max-w-xs truncate">
                                  {job.security_analysis.substring(0, 80)}...
                                </div>
                              )}
                            </div>
                          ) : job.status === 'failed' && job.error_message ? (
