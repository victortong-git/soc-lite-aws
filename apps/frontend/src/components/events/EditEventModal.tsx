...
export const EditEventModal: React.FC<EditEventModalProps> = ({ isOpen, onClose, event }) => {
  ...
  useEffect(() => {
    if (event) {
      setFormData({
        ...
        http_method: event.http_method || 'GET',
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!event || !formData.source_ip || !formData.uri) {
    ...
    try {
      ...
      setError(err || 'Failed to update event');
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Edit Event #${event.id}`}>
